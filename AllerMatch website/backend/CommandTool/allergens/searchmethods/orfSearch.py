...
def find_orfs_with_multiple_nucl(seqs, ids, trans_table, min_protein_length):
    """
            ...
            for frame in range(3):
                ...
                while aa_start < trans_len:
                    ...
                    if aa_end - aa_start >= min_protein_length:
                        ...
                        if met_pos >= 0:
                            if len(trans[aa_start+met_pos:aa_end]) >= min_protein_length:
                                answer.append((">"+i+"_ORF"+str(orf_counter), trans[aa_start+met_pos:aa_end], len(trans[aa_start+met_pos:aa_end]), strand, frame+1, start, end, i))
                                orf_counter += 1
                    aa_start = aa_end + 1
        orf_counter = 1
    return answer
