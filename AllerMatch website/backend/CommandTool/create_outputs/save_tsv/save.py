...
def save_orf_file(orf_list, input_file):
    """
    ...
    with open("output/"+input_file+"/ORFs.tsv", "w") as output_file:
...
def save_window_file(results_window, alignments, input_file):
    """
    ...
    with open("output/"+input_file+"/allergen_window.tsv", "w") as output_file:
...
def save_wordmatch_file(results_word, alignments, input_file):
    """
    ...
    with open("output/"+input_file+"/allergen_word.tsv", "w") as output_file:
        output_file.write("# To see the alignments in change font to mono-style(example: Excel = Courier, LibreOffice = FreeMono\n")
...
def save_allergen_full(results_full, input_file):
    """
    ...
    with open("output/"+input_file+"/allergen_full_fasta.tsv", "w") as output_file:
        output_file.write("# To see the alignments in change font to mono-style(example: Excel = Courier, LibreOffice = FreeMono\n")
...
def save_identical_celiac(result_identical, alignments_identical, input_file):
    db = celiac_database.import_database("")
    
    with open("output/"+input_file+"/celiac_identical.tsv", "w") as output_file:
...
def save_full_celiac(results_partial, input_file):
    db = celiac_database.import_database("")
    
    with open("output/"+input_file+"/celiac_partial.tsv", "w") as output_file:
...
def save_motif(results_motif, total_hits, input_file):
    
    with open("output/"+input_file+"/celiac_motif.tsv", "w") as output_file:
...
def save_toxin_full(results_toxins, input_file):
    
    with open("output/"+input_file+"/toxin_results.tsv", "w") as output_file:
        output_file.write('\t'.join(["Query_id", "Query_length", "Hit_id", "Hit_length", "Identity", "Similarity", "Overlap", "Bits", "E-value", "Alignments", '\n']))
